...
export default function Products() {
  ...
  // New State for Lightbox Gallery
  ...
  if (loading) return <div className="min-h-screen pt-32 text-center">Loading Products...</div>;
              ...
              <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} className="relative w-full max-w-5xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
                ...
                <div className="w-full md:w-1/2 bg-slate-50 dark:bg-slate-950 p-6 flex flex-col">
                  ...
                  <div className="flex-1 relative flex items-center justify-center overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 mb-4 group">
                     <img 
                       src={selectedProduct.images?.[activeImageIndex] || "https://via.placeholder.com/400"} 
                       className="w-full h-full object-contain p-4 transition-transform duration-500 group-hover:scale-125 cursor-zoom-in" 
                  ...
                  <div className="flex gap-2 overflow-x-auto py-2 no-scrollbar justify-center">
                     {selectedProduct.images?.map((img, idx) => (
                       <button 
                         ...
                         className={`w-16 h-16 rounded-lg border-2 overflow-hidden flex-shrink-0 ${activeImageIndex === idx ? 'border-medical-500' : 'border-transparent opacity-60 hover:opacity-100'}`}
                       >
                ...
                <div className="w-full md:w-1/2 p-6 md:p-10 overflow-y-auto">
                  ...
                  <div className="space-y-6">
                    <div>
                      <h4 className="text-xs font-bold uppercase text-slate-400 mb-1">Uses</h4>
                      <p className="text-slate-700 dark:text-slate-300 text-lg">{selectedProduct.uses}</p>
                    </div>
                    <div>
                    ...
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 p-3 rounded-lg flex gap-3 items-start">
                      <AlertCircle className="text-red-500 shrink-0 mt-0.5" size={18} />
                      <p className="text-xs text-red-600 dark:text-red-300 leading-relaxed">
                        <strong>Disclaimer:</strong> Schedule H drug. To be sold by retail on the prescription of a Registered Medical Practitioner only. B2B Info only.
                      </p>
                    </div>
                    <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
                      <button onClick={() => navigate('/contact', { state: { prefill: selectedProduct.name } })} className="w-full py-4 bg-medical-600 hover:bg-medical-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-medical-500/25">
                        Add to Inquiry & Contact
                      </button>
                    </div>
                  </div>
                </div>
