...
import { ArrowRight, X, Pill } from 'lucide-react';
...
export default function Home() {
  ...
  const fetchData = async () => {
    // Fetch Top Sellers & Articles in parallel for speed
    const [productsRes, articlesRes] = await Promise.all([
      supabase.from('products').select('*').eq('is_top_seller', true).limit(3),
  ...
  // SKELETON LOADER FOR HOME PAGE CARDS
  const HomeSkeleton = () => (
    <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 animate-pulse">
    ...
    <>
      ...
      <section className="py-20 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {loading ? (
               // Show 3 Skeletons
               [...Array(3)].map((_, i) => <HomeSkeleton key={i} />)
            ) : topProducts.length === 0 ? (
               <div className="col-span-3 text-center py-10 text-slate-500 bg-slate-50 dark:bg-slate-900 rounded-xl">
                 No products marked as "Top Seller" yet.
               </div>
            ) : (
               topProducts.map(product => (
                <div key={product.id} className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800">
                  ...
                  {/* DEEP LINKING: Sends ID to Products Page */}
                  <Link 
                    to="/products" 
      ...
      <section className="py-20 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {loading ? (
               // Show 3 Skeletons for Articles too
               [...Array(3)].map((_, i) => <HomeSkeleton key={i} />)
            ) : (
      ...
      {/* Article Modal (Unchanged Logic) */}
      <AnimatePresence>
        {selectedArticle && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedArticle(null)} className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
              <button onClick={() => setSelectedArticle(null)} className="absolute top-4 right-4 z-10 p-2 bg-black/20 hover:bg-black/40 rounded-full text-white backdrop-blur"><X size={20} /></button>
              ...
              <div className="p-8 overflow-y-auto">
                ...
                <div className="prose dark:prose-invert max-w-none text-slate-600 dark:text-slate-300 space-y-4 whitespace-pre-line leading-relaxed">{selectedArticle.content}</div>
                {selectedArticle.recommended_product && (
                  <div className="mt-10 p-6 bg-medical-50 dark:bg-medical-900/20 rounded-xl border border-medical-100 dark:border-medical-800 flex flex-col sm:flex-row items-center gap-4">
                    <div className="p-3 bg-white dark:bg-slate-800 rounded-full shadow-sm text-medical-500"><Pill size={32} /></div>
                    ...
                    <button onClick={() => navigate('/products')} className="px-6 py-2 bg-medical-600 text-white font-semibold rounded-lg hover:bg-medical-700 transition-colors shadow-lg shadow-medical-500/20">View Product</button>
                  </div>
                )}
              </div>
            </motion.div>
